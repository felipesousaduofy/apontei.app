...
/**
 * Intervalo de descanso combinado (ex.: almoço), em minutos desde a
 * meia-noite — ou null quando está desligado ou os horários não fazem
 * sentido (fim antes ou igual ao início).
 */
export function intervaloDescanso(config) {
  if (!config?.intervalo_ativo) return null;
  const ini = hmParaMin(config.intervalo_inicio);
  const fim = hmParaMin(config.intervalo_fim);
  if (ini === null || fim === null || fim <= ini) return null;
  return [ini, fim];
}

/** Recorta [di,df] de dentro de [a,b], devolvendo 0, 1 ou 2 pedaços. */
function recortar([a, b], [di, df]) {
  if (df <= a || di >= b) return [[a, b]];   // não se tocam
  if (di <= a && df >= b) return [];          // cobre o intervalo inteiro
  const pedacos = [];
  if (a < di) pedacos.push([a, di]);
  if (b > df) pedacos.push([df, b]);
  return pedacos;
}
...
export function calcularLacunas(itens, janela, dia, config) {
  ...
  // o intervalo combinado (ex.: almoço) não é "tempo sem registro": ninguém
  // deveria apontar nele, então sai da lista de lacunas antes de chegar à tela
  const descanso = intervaloDescanso(config);
  if (!descanso) return lacunas;
  return lacunas
    .flatMap(iv => recortar(iv, descanso))
    .filter(([a, b]) => b - a >= 10);
}

/**
 * Dias que contam para o resumo — a mesma regra da régua: no modo semana,
 * sábado e domingo só entram se tiveram registro, senão um fim de semana
 * livre viraria "10 horas sem apontar".
 */
function diasQueContam(itens, dias, modo) {
  if (modo !== 'semana') return dias;
  return dias.filter((d, i) => i < 5 || doDia(itens, d).length > 0);
}

/** Números do topo do diário: apontado, jornada, lacunas e volume. */
export function resumoDoPeriodo(itens, dias, modo, config) {
  const janela = janelaDoDia(itens, dias, config);
  const contam = diasQueContam(itens, dias, modo);

  const total = itens.reduce((s, l) => s + duracaoMin(l), 0);

  let lacunas = 0;
  let minutosLacuna = 0;
  contam.forEach(d => {
    calcularLacunas(doDia(itens, d), janela, d, config).forEach(([a, b]) => {
      lacunas += 1;
      minutosLacuna += b - a;
    });
  });
  ...
  const descanso = intervaloDescanso(config);
  const duracaoDescanso = descanso ? descanso[1] - descanso[0] : 0;

  const jornadaDia = Math.max(
    0,
    (hmParaMin(config.fim_dia) ?? 1080) - (hmParaMin(config.inicio_dia) ?? 480) - duracaoDescanso
  );
  const meta = jornadaDia * (modo === 'semana' ? Math.max(1, contam.length) : 1);

  return {
    total,
    meta,
    percentual: meta > 0 ? Math.round((total / meta) * 100) : 0,
    lacunas,
    minutosLacuna,
    atividades: itens.length,
    diasComRegistro: dias.filter(d => doDia(itens, d).length > 0).length,
    temDescanso: !!descanso,
    duracaoDescanso
  };
}
